Instance: ANWBronhouderCapabilityStatement-v1.2.0
InstanceOf: CapabilityStatement
Usage: #definition
Title: "ANW Bronhouder Capability Statement v1.2.0"
Description: "Capability statement describing the basic requirements for ANW FHIR servers  (version 1.2.0, ANW-bronhouder)"
...
// Breaking the glass (BTG)

// Patient: BTG search via the named query anw (_query=anw)
* rest.resource[0].searchParam[1].name = "name"
* rest.resource[0].searchParam[1].type = #string
* rest.resource[0].searchParam[1].documentation = "BTG (_query=anw): patient name filter. At least one of name or birthdate must be provided."
* rest.resource[0].searchParam[2].name = "birthdate"
* rest.resource[0].searchParam[2].type = #date
* rest.resource[0].searchParam[2].documentation = "BTG (_query=anw): patient birth date filter. At least one of name or birthdate must be provided."
* rest.resource[0].searchParam[3].name = "_elements"
* rest.resource[0].searchParam[3].type = #string
* rest.resource[0].searchParam[3].documentation = "BTG (_query=anw): data minimalization. The consumer limits the response to name, address and birthDate; presence of this parameter is validated by the bronhouder."

// Task: BTG authorization request submitted by the ANW-Zorgverlener
* rest.resource[14].type = #Task
* rest.resource[14].profile = "https://nuts.nl/fhir/StructureDefinition/nl-core-authorization-request"
* rest.resource[14].interaction[0].code = #create
* rest.resource[14].interaction[0].documentation = "Submission of a BTG authorization request (Task.code = ANW-BTG-autorisatie-verzoek) by the ANW-Zorgverlener, containing a mandatory reason.text and the requested restriction.period."

// Named query anw (BTG patient search)
* rest.operation[3].name = "anw"
* rest.operation[3].definition = "http://nuts-foundation.github.io/nl-generic-anw-ig/OperationDefinition/anw"